...
export const getPokemons = async (startIndex, endIndex) => {
  ...
  return await Promise.all(requests).then((pokemonData) =>
    pokemonData.map((p) => p.data)
  );
};
...
export const getPokemonExtraData = async () => {
  ...
  let pokemonUrls = await Promise.all([
    pokemonRequestEvolutionsUrls,
    pokemonGamesRequestUrls,
    pokemonRequestLocationsUrls,
  ]);

  // Get all data from required URLS
  let pokemonEvolutions = getRequestsLoop(
    pokemonUrls[0].data.results.map((c) => c.url)
  );
  let pokemonGames = getRequestsLoop(
    pokemonUrls[1].data.results.map((c) => c.url)
  );

  // Sleep before sending lots of requests - reduce errors from API
  await sleep(1000);
  let pokemonLocations = getRequestsLoop(
    pokemonUrls[2].data.results.map((c) => c.url)
  );
